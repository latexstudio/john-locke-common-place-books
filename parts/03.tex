...
\savebox{\dropcap}{\scalefont{2.8}\itshape B}
\nointerlineskip
\smash{\raisebox%
        {-\dimexpr \dropcapheight+0.6\thefontsize\relax}[0pt][0pt]%
        {\noindent\usebox{\dropcap}}%
}%
...
0pt \textwidth
\noindent{\itshape
    Efore we come to the Matter in Hand,
    it may not be amiss= to remark that this= Method is= put in the same Order that the Collections= ought to be put in.
    You will perceive by the Reading of that which follows= what the Heads= mean,
    which you see at the Top of the Back of every Leaf,
...
\place{EBIONITES.}
In the Gospel of the \emph{Ebionites=},
which they called the Gospel according to the \emph{Hebrews=},
the History which is= in Matthew XIX.\ 16. and the following ones=, was= thus= alter'd;
\emph{One of the Rich men said unto him,
Master, What good thing must I do that I may have Eternal Life?
Jesus= said unto him, obey the Law and the Prophets=.
He answered, I have done it.
Jesus= said unto him, go sell}\label{place:EI1}
\marginpar{\flushright\raisebox{0pt}{\color{red}\pageref{place:EI2}}}
\clearpage
\newpage

\newsavebox\verte
\savebox{\verte}{\bfseries V}
\fancyfoot[RO]{
    \begin{tikzpicture}[overlay, black, every node/.style={draw=none}]
        \node[inner sep = 0pt] (verte)
            [anchor=south west] at (2ex, 2.75\thefontsize)
            {\usebox\verte};
    \end{tikzpicture}
    \begin{tikzpicture}[overlay, black, every node/.style={draw=none}]
        \node[align=right, inner sep = 0pt] (next word)
...
\newsavebox\placebox
\savebox{\placebox}{\itshape ADVERSARIORUM METHODUS.}
\fancyhead[LE]{
    \begin{tikzpicture}[overlay, black, every node/.style={draw=none}]
        \node[inner sep = 0pt] (verte)
            [anchor=south west] at (0, 0)
            {\usebox\verte};
        \node[inner sep = 0pt] (place)
            [anchor=north west] at (-3ex, -0.7\thefontsize)
            {\usebox\placebox};
    \end{tikzpicture}
    \raisebox{-\thefontsize}{}
}
\marginpar{\footnotesize\raggedright\itshape \emph{Adversariorum Methodus=},
or the Method of \emph{Common Places=}.}\label{place:AE1}
I take a White Paper Book of what Size I think fit,
I divide the Two First Pages= which face one another,
by parallel Lines=, into Five and Twenty equal parts=, with Black Lead;
after that I cut them perpendicularly by other Lines=,
which I draw from the Top of the Page to the Bottom,
as= you may see in the Table or Index,
which I have put before this= Writing.
...
[{\itshape The other Lines= are made with Red Lead,
...
When I meet with any thing worth putting into my Common-Place-Book,
...
and the \emph{Head} \textothersc{Epistle} at the Top of the Margin of the Second Page,
...
After that, this= Fourth Page with the Fifth that follows= it,
...
One may also add to them a Third Book, which you may call the \emph{Science},
...
\place{ACHERUSIA.}
\emph{Pratum, ficta mortuorum Habitatio est Locus= prope Memphim,
...
\place{EBIONITES.}\label{place:EI2}
...
\place{HERETICKS.}\label{place:HE2}
...
\clearpage
\newpage
\place{CONFESSION OF FAITH.}\label{place:CO2}
